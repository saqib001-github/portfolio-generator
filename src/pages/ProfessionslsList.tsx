...
export default function ProfessionalsList() {
  ...
  const loading = useSelector((st: RootState) => st.portfolio.loading);
  const error = useSelector((st: RootState) => st.portfolio.error);
          ...
          <>
            <div className="mb-8 flex flex-col md:flex-row gap-3 md:gap-4">
              ...
              <Input
                ...
                value={filter}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  dispatch(setFilterBy(e.target.value.toLowerCase()))
                }
              />
            </div>
            ...
            <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
              ...
              {filtered.map((p, i) => (
                <div
                  ...
                  key={p._id}
                  className={`group transition-all duration-250 hover:-translate-y-1 hover:shadow-2xl hover:border-yellow-400 border border-transparent ${
                    bg[i % bg.length]
...
function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input {...props} className={"input flex-1 " + (props.className ?? "")} />
  );
}
